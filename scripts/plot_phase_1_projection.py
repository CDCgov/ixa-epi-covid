...
projections_path = Path("experiments", "phase1", "projection", directory_name, "simulations")
figures_path = Path("experiments", "phase1", "projection", directory_name, "figures")
show_plots = False
...
for particle_dir in projections_path.iterdir():
    if particle_dir.is_dir():
        ...
        except (pl.exceptions.NoDataError, FileNotFoundError) as e:
            print(f"No data found for particle directory: {particle_dir}, skipping.")
...
g.figure.suptitle(
    f"Observing symptomatic cases in simulations",
    fontsize='x-large',
    fontweight='bold'
)
g.figure.subplots_adjust(top=0.85)
