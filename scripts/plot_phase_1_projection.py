...
def main(subdirectory_name: str, show_plots: bool = False):
    projections_path = Path(
        "experiments", "phase1", "projection", subdirectory_name, "simulations"
    )
    figures_path = Path(
        "experiments", "phase1", "projection", subdirectory_name, "figures"
    )
        ...
        for particle_dir in projections_path.iterdir():
            if particle_dir.is_dir():
                ...
                imported_cases_timeseries = (
                    imported_cases_timeseries.with_columns(
                ...
                aggregated_deaths_report = (
                    aggregated_deaths_report.with_columns(
                        pl.col("count")
                        .cum_sum()
                ...
                imported_cases_timeseries_list.append(
                    imported_cases_timeseries
                )
    ...
    g.set_titles("")
    plt.savefig(
        figures_path / "prevalence_by_age_and_symptom_status.png", dpi=300
    )
    if show_plots:
        plt.show()
    plt.close()
...
parser = argparse.ArgumentParser(
    description="Plot results from phase 1 projection"
)
parser.add_argument(
    "-d",
    "--subdirectory-name",
    type=str,
    required=True,
    help="Subdirectory name in the projection directory to read the results from",
)
parser.add_argument(
    "-s",
    "--show-plots",
...
if __name__ == "__main__":
    ...
    main(subdirectory_name=args.subdirectory_name, show_plots=args.show_plots)
