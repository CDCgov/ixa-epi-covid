...
input_file_names = [config[x] for x in ['exe_file', 'default_params_file', 'target_csv']]
...
target_data = pl.read_csv(config['target_csv']).sort(
    "t").with_columns(
        pl.col("cases").cum_sum().alias("data_cum")
    )
...
def particles_to_params(particle, **kwargs):
    ...
    particle_dict = {
        'model_inputs': {
            'epimodel.GlobalParams': dict(particle)
            }
    }
...
def outputs_to_distance(model_output, target_data):
    symptom_report = (
        model_output
        .filter(pl.col("symptoms") == "Symptomatic")
        .group_by("t")
    ...
    time_df = pl.DataFrame(
        ...
        pl.col('time').cast(pl.Float64).alias('t')
    )

    report_output_df = symptom_report.join(
        time_df.select('t'),on='t',how='right'
    ).with_columns(
        pl.col("symptom_count").fill_null(0.0)
    ).with_columns(
        pl.col("symptom_count").cum_sum().alias("symptom_cum")
    )
    join_df = report_output_df.join(
        target_data,
        on="t",
        how="right",
    ).with_columns((pl.col("symptom_cum") - pl.col("data_cum"))
                   .abs().alias("error"))
...
sampler = ABCSampler(
    ...
    seed=config['seed'],  # Propagation of seed must be SeedSequence not int for proper pseudorandom draws
)
...
pimp_values = [p.state["probability_importation_infectious"] for p in posterior_particles.all_particles]
psymp_values = [p.state["probability_symptoms"] for p in posterior_particles.all_particles]
