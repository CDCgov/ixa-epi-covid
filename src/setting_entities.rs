...
define_rng!(SettingsEntitiesRng);

define_entity!(SettingEntity);

#[derive(Serialize, Deserialize, PartialEq, Debug, Clone, Copy)]
pub struct SettingCode(pub usize);
impl_property!(SettingCode, SettingEntity);
...
impl_property!(SettingCategory, SettingEntity);

define_property!(struct Alpha(pub f64), SettingEntity);

define_property!(struct DefaultItineraryRatio(pub f64), SettingEntity);

pub trait ContextSettingEntitiesExt:
    PluginContext + ContextRandomExt + ContextParametersExt
{
    ...
    fn add_setting(
    ...
    ) -> Result<SettingEntityId, IxaError> {
        ...
        if let Some(setting_id) = self
            .query_result_iterator::<SettingEntity, _>((SettingCode(setting_code), setting_category))
            .next()
        {
        ...
        let setting_id: SettingEntityId = self.add_entity::<SettingEntity, _>((
...
impl ContextSettingEntitiesExt for Context {}
