...
use crate::{
    infectiousness_manager::InfectionStatus, population_loader::Person
};
            ...
            if event.current == InfectionStatus::Infectious {
                if context.sample_bool(SymptomsRng, probability_mild_given_infect){
                    ...
                    if context.sample_bool(SymptomsRng, probability_severe_given_mild){
                        ...
                        let severe_time = mild_time + context.sample_distr(SymptomsRng, mild_to_severe);
                        context.add_plan(severe_time, |context|{
                        ...
                        if context.sample_bool(SymptomsRng, probability_critical_given_severe){
                            ...
                            let critical_time = severe_time + context.sample_distr(SymptomsRng, severe_to_critical);
                            context.add_plan(critical_time, |context|{
                            ...
                            if context.sample_bool(SymptomsRng, probability_dead_given_critical){
                                ...
                                let dead_time = critical_time + context.sample_distr(SymptomsRng, critical_to_dead);
                                context.add_plan(dead_time, |context|{
                                ...
                                let resolved_time = critical_time + context.sample_distr(SymptomsRng, critical_to_resolved);
                                context.add_plan(critical_time, |context|{
                            ...
                            let resolved_time = severe_time + context.sample_distr(SymptomsRng, severe_to_resolved);
                            context.add_plan(resolved_time, |context|{
                                context.set_property(event.entity_id, SymptomStatus::Resolved);
                            });
                        }
                    } else {
                        let mild_to_resolved = LogNormal::new(mild_to_resolved_mu, mild_to_resolved_sigma).unwrap();
                        let resolved_time = mild_time + context.sample_distr(SymptomsRng, mild_to_resolved);
                        context.add_plan(resolved_time, |context|{
                            context.set_property(event.entity_id, SymptomStatus::Resolved);
                        });
                    }
...
// TODO:
// -set up parameters (essential to make it run)
// -generate reports about symptom status (both incidence and prevalence are of interest, but could start with incidence)
