    ...
    PluginContext + ContextEntitiesExt + ContextSettingExtPrivate + ContextParametersExt
{
    ...
    fn sample_active_setting(&self, person_id: PersonId) -> Result<SettingId, ModelError> {
        ...
        // Conducting a weighted sample of active settings based on the ratio of time spent in each setting and the multiplier for that setting.
        // If sum of weights is 0 we sample uniformly from active settings.
        // active_settings[setting_index].0 refers to the setting_id of the sampled setting.
        if sum_weights > 0.0 {
