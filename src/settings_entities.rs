use crate::{
    itinerary::ContextItineraryExt, population_loader::PersonId
};
...
impl_property!(SettingCode, Home);
impl_property!(SettingCode, Workplace);
impl_property!(SettingCode, School);
impl_property!(SettingCode, CensusTract);

#[derive(Serialize, Deserialize, PartialEq, Debug, Clone, Copy)]
pub struct StateCode(pub usize);
impl_property!(StateCode, Home);
impl_property!(StateCode, Workplace);
impl_property!(StateCode, School);
impl_property!(StateCode, CensusTract);

#[derive(Serialize, Deserialize, PartialEq, Debug, Clone, Copy)]
pub struct Alpha(pub f64);
impl_property!(Alpha, Home);
impl_property!(Alpha, Workplace);
impl_property!(Alpha, School);
impl_property!(Alpha, CensusTract);

#[derive(Serialize, Deserialize, PartialEq, Debug, Clone, Copy)]
pub struct DefaultItineraryRatio(pub f64);
impl_property!(DefaultItineraryRatio, Home);
impl_property!(DefaultItineraryRatio, Workplace);
impl_property!(DefaultItineraryRatio, School);
impl_property!(DefaultItineraryRatio, CensusTract);
...
pub trait ContextSettingExt: PluginContext + ContextRandomExt + ContextItineraryExt {
    ...
    fn calculate_multiplier(
        ...
        let alpha = match setting {
            SettingCategory::Home(home_id) => self.get_property::<Home, Alpha>(home_id),
            SettingCategory::Workplace(workplace_id) => self.get_property::<Workplace, Alpha>(workplace_id),
            SettingCategory::School(school_id) => self.get_property::<School, Alpha>(school_id),
            SettingCategory::CensusTract(census_tract_id) => self.get_property::<CensusTract, Alpha>(census_tract_id),
        };
