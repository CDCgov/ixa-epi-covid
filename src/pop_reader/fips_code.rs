...
//! To the 54 bits apparently required to store this data we add an additional 4 bits for a category tag to distinguish
...
//! 2 bits are left unused / for future use. The least significant 6 bits are also reserved for application-specific data.
//!
...
//!  |:---------------------- | -------------:| ----------:| ---------:| -----------------:| --------------------------------------:| ---------------------:|
//!  | **Bits**               |         63…57 |      56…47 |     46…27 |             26…23 |                                   22…6 |                   5…0 |
//!  | **Ex. Value**          | `AK`, `AZ`, … |        258 |   223,100 | `Home`, `Work`, … |                                 12,345 |                     0 |
//!  | **Bit Count**          |             7 |         10 |        20 |                 4 |                                     17 |                    6 |
//!  | **Capacity**           |           128 |      1,024 | 1,048,576 |                16 |                                131,072 |                    64 |
//!  | **Decimal Digits**     |             2 |          3 |         6 |                 - |                                 3 to 6 |                     - |
//!  | **Max Observed Value** |            56 |        840 |   990,101 |                 4 |                                 42,613 |                     - |
//!
//! Observe that:
//!
//!  - We give the "category tag" 4 bits to allow up to 16 distinct categories. In some applications this field might be unused.
//!  - The least significant 6 bits are completely unused by this encoding. It may be used for application-specific storage.
...
//!  - Likewise, the unused 6 bits are the least significant bits so that numerical ordering coincides with the
...
//!  - use the 17 bits of the ID field and the unused 6 least significant bits, allowing the category tag to remain
//!    orthogonal
//!
...
const SIX_BIT_MASK: u16 = 63; // 2^6-1
const TEN_BIT_MASK: u16 = 1023; // 2^10-1
pub(crate) const SETTING_ID_MASK: IdCode = 131_071; // 2^17-1
...
impl FIPSCode {
    ...
    pub fn id(&self) -> IdCode {
        // The ID number occupies the 17 bits from bits 6..22.
        ((self.0.get() >> ID_OFFSET) as IdCode) & SETTING_ID_MASK
    }

    /// Returns the unused data region occupying the 6 least significant bits.
    #[inline(always)]
    #[must_use]
    pub fn data(&self) -> DataCode {
        self.0.get() as DataCode & SIX_BIT_MASK
    }
    // endregion Accessors
    ...
    /// Sets the unused data region occupying the 6 least significant bits in place.
    /// Returns `Ok(())` if `data` is in range, `Err(FIPSError)` otherwise.
    #[inline(always)]
    pub fn set_data_in_place(&mut self, data: DataCode) -> Result<(), FIPSError> {
        if data <= SIX_BIT_MASK {
            let inverse_mask = !(SIX_BIT_MASK as u64);
            let code = (self.0.get() & inverse_mask) | ((data & SIX_BIT_MASK) as u64);
...
mod tests {
    ...
    fn test_data_ranges() {
        ...
        assert!(FIPSCode::encode_id(SETTING_ID_MASK).is_ok());
        assert!(FIPSCode::encode_id(SETTING_ID_MASK + 1).is_err());
        assert!(FIPSCode::encode_data(SIX_BIT_MASK).is_ok());
        assert!(FIPSCode::encode_data(SIX_BIT_MASK + 1).is_err());
        // Constructors
    ...
    fn test_fips_error() {
        ...
        let e = FIPSCode::encode_id(131_072).unwrap_err();
        assert_eq!(
            e.to_string(),
            "value 131072 provided for IdCode is outside valid range of 0..131072"
        );

        let e = FIPSCode::encode_data(64).unwrap_err();
        assert_eq!(
            e.to_string(),
            "value 64 provided for DataCode is outside valid range of 0..64"
        );
    }
}
