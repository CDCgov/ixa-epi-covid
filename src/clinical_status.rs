...
use crate::{
    Params, infectiousness_manager::InfectionStatus, parameters::ContextParametersExt,
    ...
    context.subscribe_to_event(
        move |context, event: PropertyChangeEvent<Person, InfectionStatus>| {
                ...
                && context.sample_bool(SymptomRng, probability_symptoms)
            {
                ...
                let symptom_onset =
                    context.get_current_time() + context.sample_distr(SymptomRng, exp);
                context.add_plan(symptom_onset, move |context| {
                    context.set_property(
                        event.entity_id,
                        SymptomData::Symptomatic {
                            symptom_onset: context.get_current_time(),
                        },
                    );
                });
            }
        ...
        move |context, event: PropertyChangeEvent<Person, SymptomStatus>| {
            if event.current == SymptomStatus::Symptomatic {
                ...
                context.add_plan(recovery_time, move |context| {
                    ...
                    let SymptomData::Symptomatic { symptom_onset } =
