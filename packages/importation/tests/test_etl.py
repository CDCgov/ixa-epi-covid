...
import polars as pl
import pytest
from importation.etl import get_data, get_parameters, read_bytes
...
def test_get_parameters(mock_urlopen, mock_read_csv):
    # Mock response data
    mock_response = mock_urlopen.return_value.__enter__.return_value
...
def test_get_data(mock_urlopen, mock_read_csv):
    # Mock response data
    mock_response = mock_urlopen.return_value.__enter__.return_value
    mock_response.read.return_value = b"mock csv data"
    mock_read_csv.return_value = pl.DataFrame(
